...
async function withTempGitFixture(caseName, mutateFixture, assertion) {
  const tempRoot = await mkdtemp(
    path.join(os.tmpdir(), 'skill-validator-fixture-'),
  );
  const fixtureRoot = path.join(fixturesRoot, caseName);
      ...
      await assert.rejects(
        ...
        (error) => {
          ...
          assert.notEqual(
            error.code,
            0,
            'deleted markdown --changed case should exit non-zero',
          );
          assert.match(
            combinedOutput,
  ...
  await withTempGitFixture(
    ...
    async (tempRoot) => {
      await rename(
        ...
        path.join(tempRoot, 'docs', 'renamed-guide.md'),
      );
      ...
      await assert.rejects(
        ...
        (error) => {
          ...
          assert.notEqual(
            error.code,
            0,
            'renamed markdown --changed case should exit non-zero',
          );
          assert.match(
            combinedOutput,
      ...
      await assert.rejects(
        ...
        (error) => {
          ...
          assert.notEqual(
            error.code,
            0,
            'renamed markdown --staged case should exit non-zero',
          );
          assert.match(
            combinedOutput,
            /Renamed tracked repository markdown docs detected; validating all skill folders and repository markdown links to re-check inbound links\./,
      ...
      await assert.rejects(
        ...
        (error) => {
          ...
          assert.notEqual(
            error.code,
            0,
            'deleted markdown --staged case should exit non-zero',
          );
          assert.match(
            combinedOutput,
            /Deleted tracked repository markdown docs detected; validating all skill folders and repository markdown links to re-check inbound links\./,
