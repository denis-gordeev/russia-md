...
不是只有 merge 和 reject。對善意貢獻者，省掉一來一回比堅持完美更重要。
...
- 你 merge 後花 10 分鐘就能修好 → 🔧 merge + 自己修
- 需要花 30 分鐘以上或重寫 >50% → ❌ request changes
- 開源社群最怕「熱情被冷水潑」。merge + 自己修 = 貢獻者得到正面回饋，你得到品質保證。雙贏。

### 翻譯 PR 的上游檢查（經驗萃取鐵律）

**審翻譯 PR 前，先檢查原文品質：**

1. 原文有腳註嗎？→ 如果沒有，翻譯缺腳註不是翻譯者的錯
2. 原文的 category/slug 是什麼？→ 翻譯應保持一致，不一致的話你自己修
3. 問題是個案還是系統性的？→ 如果原文就有病，治原文比治翻譯優先
