...
| **7.5 — Template Migrations** | 5 個 src/templates style 區塊    | 🟡 部分完成 | 2026-04-10 | 2026-04-10 | merged to main (partial) |
...
1. **共用 class 要不就全遷、要不就全留**：about.template.astro 的 `.section-title` / `.section-subtitle` 被六個 section 共用，只遷一個 section 就得複製全部樣式。這種情況下部分遷移沒有意義——要嘛全 refactor，要嘛先不動。Phase 7.5 把它列為「延後」而非「部分完成」。
