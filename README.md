...
- [ ] Add fixture coverage for hidden per-file truncation summaries when more than five files are suppressed at once
- [ ] Make the hidden-file truncation summary limit configurable separately from the visible markdown error cap

## Latest Round

- Added per-file hidden-summary lines to truncated markdown validation output, so capped runs still show which source files retain suppressed failures.
- Extended fixture coverage for path-scoped truncation, invalid `--markdown-error-limit` CLI/env input, and grouped truncation overrides driven by both CLI args and `SKILL_VALIDATOR_MAX_MARKDOWN_ERRORS`.
...
- `scripts/validate-skill-examples.mjs` local and CI validator for skill bundle completeness, metadata schema checks, markdown-link and anchor validation with nearest-match suggestions, grouped/capped aggregate markdown diagnostics with per-file hidden summaries, optional icon asset path linting, changed-skill filtering, and directory-aware `--paths` expansion for tracked markdown trees
- `scripts/test-validate-skill-examples.mjs` validator fixture runner covering valid and negative bundle cases, including anchor-suggestion and no-suggestion fallbacks, malformed-agent-metadata diagnostics, front-matter line-number regressions, grouped markdown truncation with configurable limits and invalid-input handling, and directory-scoped `--paths` regressions
