...
- [ ] Add CRLF fixture coverage for duplicate explicit HTML anchor diagnostics after front matter
- [ ] Reject empty or whitespace-only explicit HTML anchor IDs with line-aware diagnostics
...
- [ ] Detect collisions between explicit HTML IDs and generated heading slugs, not only duplicate explicit IDs

## Latest Round

- Added duplicate explicit HTML anchor detection so repeated `id="..."` values now fail validation before local links become ambiguous in the rendered site.
- Made duplicate-anchor diagnostics file-and-line-aware, including the original definition line, and covered aggregation across repository docs and per-skill integration notes.
- Extended skill validation to check links and explicit-anchor integrity inside each required `references/integration-notes.md`, not only the top-level `SKILL.md`.
- Ignored anchor-like markup inside inline code, fenced examples, and HTML comments, plus repeated `data-id` attributes, while preserving real explicit IDs and whitespace around `id = "..."`.
