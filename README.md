...
- [ ] Keep nearest-anchor family ranking stable when mixed explicit HTML `id=""` anchors and repeated heading slugs coexist in one document
- [ ] Add fixture coverage for nearest-anchor family ranking in front-matter-heavy markdown files with repeated headings
- [ ] Add a small validator helper test for repeated-heading slug-family ordering so future truncation or scoring tweaks do not reshuffle suggestions silently
...
`npm run check:skills:fixtures` runs a small fixture suite for the validator itself, including front-matter-aware markdown regression coverage plus negative cases for broken markdown anchors with nearest-anchor suggestions and no-suggestion fallbacks, slug-family ordering for repeated-heading anchor suggestions, malformed YAML front matter, malformed `agents/openai.yaml` files with file-local line/column diagnostics, malformed markdown-link syntax that should be ignored safely, aggregated markdown-link failures with line-aware diagnostics, grouped/capped markdown error output when one run surfaces many broken links, explicit coverage for unlimited and low-limit `--markdown-error-limit` overrides, invalid CLI/env markdown-error-limit and markdown-hidden-file-limit input, grouped truncation overrides through both CLI args and `SKILL_VALIDATOR_MAX_MARKDOWN_ERRORS`, per-file hidden-summary output for truncated markdown groups including runs that suppress more than five source files, explicit path-scoped hidden-file-limit coverage for grouped truncation during `--paths` runs, dedicated regression coverage for correct broken-link line numbers in front-matter-heavy markdown files, missing shared metadata schema files, invalid `interface.icon` metadata, path-scoped no-op output when `--paths` selects only ignored or unmatched files including mixed ignored-markdown and unmatched selections, path-scoped truncated markdown diagnostics, no-op coverage for `--changed` and `--staged` runs that touch only out-of-scope files, truncated diagnostics for large ignored/unmatched `--paths` selections, and directory-scoped `--paths` coverage for tracked repository markdown trees as well as direct skill-folder selections.
