...
- [ ] Detect explicit HTML anchor IDs whose opening tags or attributes span multiple lines
- [ ] Add generated-anchor validation for Setext-style Markdown headings
...
- Rejected empty and whitespace-only explicit HTML `id` attributes with file-and-line-aware diagnostics instead of silently ignoring them.
- Detected collisions in both definition orders between explicit HTML IDs and generated ATX heading slugs, including the original definition line in each diagnostic.
- Preserved duplicate-explicit-ID reporting while sharing the same scan with heading collision checks, fenced-code, inline-code, and HTML-comment exclusions.
- Added fixture coverage for empty IDs, whitespace-only IDs, explicit-ID-first collisions, heading-first collisions, and CRLF duplicate-ID diagnostics after YAML front matter.
- Kept valid anchor collection free of empty IDs so malformed definitions cannot accidentally satisfy local fragment links.
