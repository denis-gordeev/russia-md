...
- [ ] Add fixtures for collisions involving repeated heading suffixes such as `#guide-1`
...
- [ ] Recognize legacy `<a name="...">` anchors with the same duplicate and collision safeguards as `id="..."`
- [ ] Add structured JSON diagnostics for CI annotations and editor integrations
- [ ] Add malformed multi-line HTML tag fixtures so incomplete attributes cannot leak into adjacent Markdown

## Latest Round

- Added document-wide collection of explicit HTML `id` anchors after Markdown comments, inline code, and fenced code are masked.
- Made local fragment links resolve IDs whose tag and attributes span multiple lines.
- Preserved physical source-line diagnostics by reporting the line where each multi-line `id` attribute starts.
- Extended duplicate-ID and generated-heading collision checks to multi-line HTML tags.
- Added regression coverage for valid multi-line anchors, single- and double-quoted duplicate IDs, heading collisions, and ignored multi-line markup inside comments and fenced code.
