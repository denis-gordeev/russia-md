...
- [x] Add anchor-cache invalidation coverage for validator helper imports and repeated in-process validation runs
- [x] Add local-link coverage for percent-encoded file paths and fragments that mix encoded and literal Unicode
...
- [x] Detect explicit HTML anchor IDs whose opening tags or attributes span multiple lines
- [x] Add generated-anchor validation for Setext-style Markdown headings
- [x] Add fixtures for collisions involving repeated heading suffixes such as `#guide-1`
- [ ] Share one parsed anchor-definition index between integrity checks and local-link resolution
- [ ] Add local-link validation for reference-style Markdown links and definitions
- [ ] Add Setext-heading fixtures for inline formatting, escaped punctuation, and mixed ATX/Setext slug suffixes
...
- [x] Add mixed-newline fixtures for explicit-anchor parsing across LF, CRLF, and lone-CR inputs
- [ ] Add a deterministic fuzz corpus for HTML tag, quote, and entity boundary combinations
- [ ] Add percent-encoded local-path fixtures for reserved filename characters such as `%23`, `%25`, and encoded separators
- [ ] Add mixed-newline fixture coverage for Setext headings and broken-link line diagnostics after front matter
- [ ] Add malformed percent-encoding aggregation coverage when local paths and fragments fail in the same document
- [ ] Add cache-behavior coverage when multiple source documents resolve anchors from the same edited Markdown target in one process
- [ ] Add direct helper coverage for anchor-cache invalidation after front matter changes without a pathname change

## Latest Round

- Made the Markdown anchor cache content-aware so imported validator helpers cannot reuse stale anchors after an in-process file rewrite.
- Exported the Markdown-link validation helper and added a repeated-run regression that rewrites one same-sized file between valid anchor sets.
- Normalized Markdown line splitting and source-line counting across LF, CRLF, and lone-CR inputs so mixed-newline files retain correct explicit-anchor offsets.
- Added direct parser-helper and end-to-end duplicate-anchor regressions for mixed newline styles.
- Decoded percent-encoded local file paths before filesystem resolution, including paths that mix encoded Cyrillic with literal Unicode.
- Added a dedicated malformed local-path encoding diagnostic instead of reporting an encoded path as an ordinary missing file.
...
`npm run check:skills` now validates that every repo-local skill ships its core bundle together: `SKILL.md`, `agents/openai.yaml`, `references/integration-notes.md`, `examples/output.json`, and `schemas/output.schema.json`. It also validates `agents/openai.yaml` against a shared metadata schema, rejects unknown UI keys, checks that each default prompt explicitly mentions the matching `$skill-name`, verifies local markdown links across `README.md`, `skills/**/SKILL.md`, required per-skill integration notes, and shared docs/reference pages, ignores YAML front matter while scanning markdown bodies, validates local markdown `#fragment` anchors against ATX and Setext headings plus tag-aware explicit `id=""` and legacy `<a name="">` anchors whose tags and attributes may span lines, tracks physical source lines across LF, CRLF, and lone-CR inputs, decodes percent-encoded local paths and fragments including mixed encoded/literal Unicode, decodes HTML character references in explicit anchor values before link resolution and collision checks, rejects empty or duplicate explicit HTML anchors, detects collisions between explicit anchors and generated heading slugs with original definition lines, ignores incomplete multi-line tags before they can consume adjacent Markdown, reports malformed percent escapes and broken markdown references with physical source line numbers after front matter, groups repeated markdown failures by source file before capped output is truncated, adds per-file hidden summaries when grouped truncation suppresses later files, lets the hidden-file summary cap be overridden separately from the visible markdown error cap while surfacing the active hidden-file summary limit directly in truncated diagnostics, fails fast on invalid markdown-error-limit and markdown-hidden-file-limit CLI or environment input, suggests the nearest matching anchor when a markdown fragment misses by a small typo, and lints local `interface.icon` asset paths when icons are present.
...
`npm run check:skills:fixtures` runs a small fixture suite for the validator itself, including front-matter-aware markdown regression coverage plus CRLF variants for Windows-authored files, mixed LF/CRLF/lone-CR explicit-anchor parsing and duplicate diagnostics, repeated imported-helper validation with content-aware anchor-cache invalidation after same-sized in-process file rewrites, ATX and Setext generated-anchor resolution, repeated Setext heading suffixes and their explicit-anchor collisions in both definition orders, percent-encoded non-ASCII path and anchor resolution with mixed literal Unicode, malformed path and fragment escape diagnostics, empty-anchor rejection, single- and multi-line explicit HTML `id` and legacy `<a name>` resolution, self-closing and void-element anchor parsing, named and numeric HTML entity decoding before fragment and duplicate checks, a direct exported-parser helper regression that avoids fixture-repository setup, duplicate explicit HTML anchor aggregation across repository docs and per-skill integration notes, explicit-anchor/generated-heading collision checks in both definition orders for ATX and Setext headings, malformed multi-line tag isolation, CRLF duplicate-ID diagnostics after front matter, inline-code, fenced-code, HTML-comment, `data-id`, and non-`a` `name` exclusions including multi-line ignored markup, whitespace-tolerant HTML anchor parsing, negative cases for broken markdown anchors with nearest-anchor suggestions and no-suggestion fallbacks, slug-family ordering for repeated-heading and explicit-HTML anchor suggestions, malformed YAML front matter, malformed `agents/openai.yaml` files with file-local line/column diagnostics, malformed markdown-link syntax that should be ignored safely, aggregated markdown-link failures with line-aware diagnostics, grouped/capped markdown error output when one run surfaces many broken links, explicit coverage for unlimited and low-limit `--markdown-error-limit` overrides, invalid CLI/env markdown-error-limit and markdown-hidden-file-limit input, grouped truncation overrides through both CLI args and `SKILL_VALIDATOR_MAX_MARKDOWN_ERRORS`, per-file hidden-summary output for truncated markdown groups including runs that suppress more than five source files, explicit path-scoped hidden-file-limit coverage for grouped truncation during `--paths` runs, dedicated regression coverage for multiple exact broken-link line numbers in one front-matter-heavy markdown file, valid explicit HTML anchors after front matter, missing shared metadata schema files, invalid `interface.icon` metadata, path-scoped no-op output when `--paths` selects only ignored or unmatched files including mixed ignored-markdown and unmatched selections, path-scoped truncated markdown diagnostics, no-op coverage for `--changed` and `--staged` runs that touch only out-of-scope files, truncated diagnostics for large ignored/unmatched `--paths` selections, and directory-scoped `--paths` coverage for tracked repository markdown trees as well as direct skill-folder selections.
...
- `scripts/validate-skill-examples.mjs` local and CI validator for skill bundle completeness, metadata schema checks, markdown-link and anchor validation with ATX/Setext headings plus tag-aware multi-line `id` and legacy `<a name>` support, browser-equivalent HTML entity decoding, incomplete-tag isolation, empty/duplicate explicit-anchor rejection and generated-heading collision detection, nearest-match suggestions, and repeated-heading slug-family ranking, grouped/capped aggregate markdown diagnostics with per-file hidden summaries, a separate hidden-file summary cap, and inline reporting of the active hidden-file summary limit, optional icon asset path linting, changed-skill filtering, and directory-aware `--paths` expansion for tracked markdown trees
- `scripts/test-validate-skill-examples.mjs` validator fixture and exported-helper runner covering valid and negative bundle cases, including content-aware anchor-cache invalidation across repeated in-process file rewrites, ATX/Setext fragment resolution, repeated Setext slug suffixes and their collision ordering, single- and multi-line explicit-ID and legacy-name diagnostics, self-closing and void-element anchors, HTML character-reference decoding, incomplete-tag isolation, explicit-anchor/generated-heading collisions, markdown-code/comment exclusions, anchor-suggestion ordering and no-suggestion fallbacks, malformed-agent-metadata diagnostics, LF/CRLF front-matter line-number regressions, encoded Unicode anchors and malformed percent escapes, grouped markdown truncation with configurable visible-error and hidden-file-summary limits plus invalid-input handling, and directory-scoped `--paths` regressions including path-scoped hidden-file-limit coverage
